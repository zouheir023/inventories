...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
  ...
  function page_require_level($require_level)
  {
    ...
    // If the user's group status is "Deactive" (0)
    elseif (!isset($login_level['group_status']) || $login_level['group_status'] === '0') {
      $session->msg('d', 'This level user has been banned!');
      redirect('home.php', false);
    }
  
    // Checking if the log-in user level is less than or equal to the required level
    elseif ($current_user['user_level'] <= (int)$require_level) {
      return null;
    } else {
      $session->msg('d', 'Sorry! You do not have permission to view this page.');
      redirect('home.php', false);
    }
  }
  ...
  function find_name_all_product(){
    global $db;
...
function  dailySales($year,$month,$day){
  ...
  $sql .= " FROM sales s";
  $sql .= " LEFT JOIN products p ON s.product_id = p.id";
  $sql .= " WHERE DATE_FORMAT(s.date, '%Y-%m-%e' ) = '{$year}-{$month}-{$day}'";
